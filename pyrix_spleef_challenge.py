...
"""
class my_Clock:
	def __init__(self, id, event, x, z):
		self.id = id
		self.event = event
		self.x = x
		self.z = z
...
def tick(event):
	...
	if event.npc.world.getClosestEntity(event.npc.getPos(), 49, 1) != None: #someone is nearby, let's do some nasty shit
		...
		x = int(math.cos(a) * r) + x_c
		z = int(math.sin(a) * r) + z_c
		...
		if d_10 == 0 or d_10 == 1: # magma blocks
			...
			event.npc.executeCommand(command)
			id = int(time.clock()) # it's actually the "time", but I'm using it as a unique identifier for this particular object
		...
		# note...this causes significant lag for several seconds.  *shrug*  it *is* a command that is essentially activated 10k times...
		# maybe someday I'll figure out a more efficient way of doing this
		#event.npc.timers.clear()
		#event_thingie = {} # garbage collection #### nope, this causes the NPC to leave the "warning blocks" behind. :-/
		# do a nested for loop that replaces all the holes with something solid
		if rebuild_complete == False:
			for x_this in range(-50,50):
				for z_this in range(-50,50):
					h = math.sqrt(x_this ** 2 + z_this ** 2)
					if h < 50:
